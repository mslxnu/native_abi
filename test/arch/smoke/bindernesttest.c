/* freestanding: one thread can be answering two calls at once, and both of the
 * callers have to be answered.
 *
 * Binder is reentrant. libbinder answers incoming transactions while it waits
 * for the reply to a call of its own, so a server thread part way through
 * answering A is routinely handed B before A is answered. It then owes two
 * replies, and they come back in the reverse order to the calls. Linux keeps
 * exactly that, per thread, as the transaction stack.
 *
 * nabi kept one slot per thread and overwrote it, so a nested call erased the
 * outer one. The reply to B took the slot and reached B's caller; the reply to
 * A then found no slot, fell back to the replying endpoint itself, and A's
 * caller was left waiting for a reply that had gone to the wrong process. That
 * is a boot which stops with a reply owed and never collected, in whichever
 * service happened to nest.
 *
 * So: a manager takes two calls before it answers either of them, and both
 * callers must come away with a reply.
 */
...
/* 10ms apiece, so a caller waits about four seconds before giving up on a
 * reply. Long enough that a slow answer is not read as a lost one. */
#define TRIES 400
...
/* How many of `cmd` are in this batch. A read can carry both calls at once, so
 * counting them is not the same as noticing one. */
static int count_cmd(const unsigned char *rbuf, unsigned long consumed, unsigned int cmd)
{
  int n = 0;
...
/* Send one call to the context manager and do not wait for it here. */
static long send_call(int fd, unsigned int code)
{
  ...
  *(unsigned int *)(wbuf + woff) = BC_TRANSACTION; woff += 4;
  ...
  woff += (int) sizeof tr;
  return bwr(fd, wbuf, woff, 0, 0, 0);
}

static int poll_reply(int fd)
{
  unsigned char rbuf[1024];
  unsigned long consumed;
  for (int i = 0; i < TRIES; i++) {
    consumed = 0;
    if (bwr(fd, 0, 0, rbuf, sizeof rbuf, &consumed) == 0 &&
        count_cmd(rbuf, consumed, BR_REPLY) > 0)
      return 1;
    naptime(10);
  }
  return 0;
...
/*
 * Takes two calls before answering either, which is the whole point: between
 * the two BR_TRANSACTIONs this one thread is inside both of them.
 */
static void manager(int ready_w)
{
  ...
  *(unsigned int *)wbuf = BC_ENTER_LOOPER;
  ...
  for (int spin = 0; spin < TRIES && held < 2; spin++) {
    consumed = 0;
    if (bwr(fd, 0, 0, rbuf, sizeof rbuf, &consumed) != 0) break;
    held += count_cmd(rbuf, consumed, BR_TRANSACTION);
    if (held < 2) naptime(10);
  }
  if (held < 2) sys6(SYS_exit_group, 15, 0,0,0,0,0);

  /* Both answers, innermost first - which is the order BC_REPLY means, since
   * it names no target and the driver is expected to know. */
  for (int i = 0; i < 2; i++) {
    struct btd tr;
    mzero(&tr, sizeof tr);
    tr.code = 0x5a;
...
static volatile int second_answered = 0;
...
/* The second caller. Same endpoint, different thread, so only the recorded
 * thread tells the two replies apart. */
__attribute__((noinline)) static void second_client(void)
{
  await(go[0]);
  if (send_call(cfd, 2) == 0)
    second_answered = poll_reply(cfd);
  wake(back[1]);
...
void _start(void)
{
  ...
  if (kid <= 0) { put("bindernest failed\n"); sys6(SYS_exit_group,1,0,0,0,0,0); }
  sys6(SYS_close, ready[1], 0,0,0,0,0);
  char b; sys6(SYS_read, ready[0], (long)&b, 1, 0,0,0);

  /* Non-blocking: a caller polling for its own reply must not block on the
   * batch that carries the other one. */
  ...
  if (tid == 0) second_client();
  want("the second caller started", tid > 0, 1);

  wake(go[1]);
  want("the first call was sent", send_call(cfd, 1), 0);
  int first_answered = poll_reply(cfd);
  await(back[0]);

  /*
   * Both, and this is the assertion: under the overwrite exactly one of them
   * came back, whichever call the manager happened to be handed second.
   */
  want("the caller of the first call was answered", first_answered, 1);
  want("the caller of the second call was answered", second_answered, 1);
